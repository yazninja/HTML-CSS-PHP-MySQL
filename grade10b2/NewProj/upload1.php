...
if ($a==Item){
		...
		mysql_query("UPDATE product (img) VALUES ('$image') where barcode='$b'");
		echo "<div class='good'><b>Sucessfully Updated Image Table for Items</b></div>";
}
else if ($a==Users){
		$b=$_POST['username'];
		$image=addslashes(file_get_contents($_FILES['Image']['tmp_name']));
			mysql_query("INSERT INTO images (username,image,date,time) VALUES ('$b','$image',curdate(),curtime())");
			$imgresult=mysql_query ("select * from  images where username='$b' order by date DESC, time DESC limit 1");
			while ($row=mysql_fetch_array($imgresult)){
			$image=$row['image'];
			echo '<div class="good"><b>Sucessfully Updated Image Table for Users</b><br><br>';
		echo "<img src='data;base64,"echo $image;"' width='175' height='200' />
		</div>";
}
}
}
